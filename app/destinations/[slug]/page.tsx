...
export async function generateMetadata({
  ...
  return {
    title: titles[params.slug] ?? 'Destination — Raizen Myanmar',
  };
}
...
// Per-destination static config (images, labels, etc.)
...
> = {
  ngwesaung: {
    ...
    galleryImages: [
      ...
      {
        src: 'https://images.unsplash.com/photo-1571003123894-1f0594d2b5d9?w=600&q=80&auto=format&fit=crop',
        alt: 'Resort Pool',
      },
      {
    ...
    bodyText: [
      ...
      'Unlike the busier Ngapali Beach, Ngwe Saung retains a quiet, authentic character. Local fishing villages, fresh seafood restaurants, and resorts from budget guesthouses to luxury bungalows.',
    ],
  ...
  'chaung-thar': {
    ...
    galleryImages: [
      ...
      {
        src: 'https://images.unsplash.com/photo-1544551763-46a013bb70d5?w=600&q=80&auto=format&fit=crop',
        alt: 'Sunset at Chaung Thar',
      },
      {
        src: 'https://images.unsplash.com/photo-1559827260-dc66d52bef19?w=600&q=80&auto=format&fit=crop',
        alt: 'Seafood by the Beach',
      },
    ],
...
export default async function DestinationPage({
  ...
  // If slug is not one we support, 404
  if (!config) notFound();
      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        <section className="mb-10">
          ...
          <div className="flex flex-wrap gap-2 mt-5">
            {config.badgeTags.map((tag) => (
              <span
                key={tag}
                className="bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-sm px-3 py-1 rounded-full font-medium"
              >
                {tag}
              </span>
            ))}
        ...
        <section className="mb-12">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mb-5">
            All prices in Myanmar Kyat per night.
          </p>
          ...
          <div className="space-y-4">
            {hotels.map((hotel) => {
              ...
              return (
                <div
                  key={hotel.id}
                  className="border border-gray-200 dark:border-gray-800 rounded-2xl p-5 hover:border-blue-300 dark:hover:border-blue-700 transition-colors"
                >
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <Link
            href="/planner"
            prefetch
            className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-center font-semibold py-3 px-6 rounded-xl transition-colors"
          >
            {config.planLabel}
          </Link>
          <Link
            href="/destinations"
            prefetch
            className="flex-1 border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 text-center font-semibold py-3 px-6 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors"
          >
            ← Back
          </Link>
        </div>
