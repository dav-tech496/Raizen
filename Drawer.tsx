      ...
      >
        <div className="flex items-center justify-between px-[18px] h-[58px] border-b border-border">
          ...
          <button
            ...
            className="w-11 h-11 rounded-[10px] bg-surface2 border border-border flex items-center justify-center text-ink2 transition-colors active:bg-border"
          >
        ...
        <nav className="flex flex-col px-[18px] pt-5 gap-1 flex-1">
          {NAV_LINKS.map(({ href, labelKey }) => (
            <Link
              ...
              href={href}
              onClick={onClose}
              className="flex items-center gap-3 px-4 py-[13px] min-h-[44px] rounded-[12px] text-[15px] font-medium text-ink hover:bg-surface2 active:bg-border transition-colors"
            >
              {t(labelKey)}
            </Link>
          ))}
          ...
          {user ? (
            <>
              ...
              <button
                ...
                className="flex items-center gap-3 px-4 py-[13px] min-h-[44px] rounded-[12px] text-[15px] font-medium text-green hover:bg-green-pale active:bg-green/10 transition-colors text-left"
              >
                {t('logout')}
              </button>
            </>
            ...
            <>
              <Link
                href="/login"
                onClick={onClose}
                className="flex items-center gap-3 px-4 py-[13px] min-h-[44px] rounded-[12px] text-[15px] font-medium text-ink hover:bg-surface2 transition-colors"
              >
                {t('login')}
              </Link>
              <Link
                href="/register"
                onClick={onClose}
                className="flex items-center gap-3 px-4 py-[13px] min-h-[44px] rounded-[12px] text-[15px] font-semibold text-white bg-green hover:bg-green-mid transition-colors mt-1"
              >
                {t('register')}
              </Link>
            </>
          ...
          <div className="flex gap-2">
            {(['en', 'mm'] as const).map((l) => (
              <button
                key={l}
                onClick={() => setLang(l)}
                className={`flex-1 py-[11px] min-h-[44px] rounded-[10px] text-sm font-semibold border transition-colors ${
                  lang === l
